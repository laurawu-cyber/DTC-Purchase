...
import svgPaths from "../../imports/svg-waee18o50u";
import { imgVector } from "../../imports/svg-bc8dx";
...
function IndustryOptionsContainer({ selectedIndustry, onSelectIndustry }: { selectedIndustry: string | null; onSelectIndustry: (industry: string) => void }) {
  const industries = [
    "Technology / Software",
        ...
        return (
          ...
          >
            <div className="content-stretch flex gap-[8px] items-center justify-center min-w-[inherit] overflow-clip px-[24px] py-[8px] relative rounded-[inherit] size-full">
              <p className={`font-['Inter:Regular',sans-serif] font-normal leading-[22px] not-italic relative shrink-0 text-[14px] ${isSelected ? 'text-black' : 'text-[#3d3d3d]'} text-center whitespace-nowrap`}>
                {industry}
              </p>
            </div>
            <div
              aria-hidden="true"
              className={`absolute border ${isSelected ? 'border-black' : 'border-[#d6d6d6]'} border-solid inset-0 pointer-events-none rounded-[5px]`}
            />
          </button>
        );
      })}
...
function IndustryContainer({ selectedIndustry, onSelectIndustry }: { selectedIndustry: string | null; onSelectIndustry: (industry: string) => void }) {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start relative shrink-0 w-full" data-name="Industry Container">
      ...
      <IndustryOptionsContainer selectedIndustry={selectedIndustry} onSelectIndustry={onSelectIndustry} />
    </div>
  );
}

function FormContainer({ selectedIndustry, onSelectIndustry }: { selectedIndustry: string | null; onSelectIndustry: (industry: string) => void }) {
  return (
    <div className="-translate-x-1/2 absolute content-stretch flex flex-col gap-[48px] items-start left-1/2 overflow-clip top-[80px] w-[464px]" data-name="Form Container">
      <WelcomeContainer />
      <IndustryContainer selectedIndustry={selectedIndustry} onSelectIndustry={onSelectIndustry} />
    </div>
  );
}
...
export default function TeamProfile2Interactive({ onClose }: { onClose?: () => void }) {
  const [selectedIndustry, setSelectedIndustry] = useState<string | null>(null);
  const navigate = useNavigate();

  const handleSelectIndustry = (industry: string) => {
    setSelectedIndustry(industry);
    // Close modal and navigate to /home-team after selection
    setTimeout(() => {
      if (onClose) {
  ...
  return (
    <div className="bg-[#f9f9f9] w-screen h-screen overflow-auto relative" data-name="team profile2">
      <CommunitySection />
      <FormContainer selectedIndustry={selectedIndustry} onSelectIndustry={handleSelectIndustry} />
      <UserNameContainer />
